...
namespace Cosmo.Utils.Drawing
{
   ...
   internal static class JpegTables
   {
      ...
      public static JpegTables.BitString[] BitCode = new JpegTables.BitString[65535];
      ...
      static Byte[] Scale_And_ZigZag_Quantization_Table(Byte[] inTable, float quality_scale)
      {
         ...
         for (Byte i = 0; i < 64; i++)
         {
            ...
            if (temp > 255L)
               temp = 255L;
            outTable[JpegTables.ZigZag[i]] = (Byte)temp;
         }
         return outTable;
      }
      ...
      {
         JpegTables.Y_Table = Scale_And_ZigZag_Quantization_Table(luminance_table, scaleFactor);
         JpegTables.CbCr_Table = Scale_And_ZigZag_Quantization_Table(chromiance_table, scaleFactor);
      }

      static void Compute_Huffman_Table(Byte[] nrCodes, Byte[] std_table, ref JpegTables.BitString[] Huffman_Table)
      {
         Byte k, j;
         Byte pos_in_table;
         UInt16 code_value;
      ...
      static void Initialize_Huffman_Tables()
      {
         ...
         Compute_Huffman_Table(JpegTables.Standard_DC_Luminance_NRCodes, JpegTables.Standard_DC_Luminance_Values, ref JpegTables.Y_DC_Huffman_Table);
         Compute_Huffman_Table(JpegTables.Standard_AC_Luminance_NRCodes, JpegTables.Standard_AC_Luminance_Values, ref JpegTables.Y_AC_Huffman_Table);
         Compute_Huffman_Table(JpegTables.Standard_DC_Chromiance_NRCodes, JpegTables.Standard_DC_Chromiance_Values, ref JpegTables.Cb_DC_Huffman_Table);
         Compute_Huffman_Table(JpegTables.Standard_AC_Chromiance_NRCodes, JpegTables.Standard_AC_Chromiance_Values, ref JpegTables.Cb_AC_Huffman_Table);
      }
      ...
      static void Initialize_FDCT_Quantization_Tables()
      {
         ...
         for (Byte row = 0; row < 8; row++)
         {
            ...
            {
               JpegTables.FDCT_Y_Quantization_Table[i] = (float)(1.0 / ((double)JpegTables.Y_Table[JpegTables.ZigZag[i]] *
                   CosineScaleFactor[row] * CosineScaleFactor[col] * 8.0));
               JpegTables.FDCT_CbCr_Quantization_Table[i] = (float)(1.0 / ((double)JpegTables.CbCr_Table[JpegTables.ZigZag[i]] *
                   CosineScaleFactor[row] * CosineScaleFactor[col] * 8.0));
               i++;
            }
         }
      ...
      public static void Precalculate_YCbCr_Tables()
      {
         ...
         for (R = 0; R < 256; R++)
         {
            JpegTables.Y_Red_Table[R] = (Int32)((65536 * 0.299 + 0.5) * R);
            JpegTables.Cb_Red_Table[R] = (Int32)((65536 * -0.16874 + 0.5) * R);
            JpegTables.Cr_Red_Table[R] = (Int32)((32768) * R);
         }
         for (G = 0; G < 256; G++)
         {
            JpegTables.Y_Green_Table[G] = (Int32)((65536 * 0.587 + 0.5) * G);
            JpegTables.Cb_Green_Table[G] = (Int32)((65536 * -0.33126 + 0.5) * G);
            JpegTables.Cr_Green_Table[G] = (Int32)((65536 * -0.41869 + 0.5) * G);
         }
         for (B = 0; B < 256; B++)
         {
            JpegTables.Y_Blue_Table[B] = (Int32)((65536 * 0.114 + 0.5) * B);
            JpegTables.Cb_Blue_Table[B] = (Int32)((32768) * B);
            JpegTables.Cr_Blue_Table[B] = (Int32)((65536 * -0.08131 + 0.5) * B);
         }
      }
