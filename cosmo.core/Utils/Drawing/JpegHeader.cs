...
namespace Cosmo.Utils.Drawing
{
   internal class JpegHeader
   {
      public class APP0infotype
      {
         ...
         {
            DrawingUtils.WriteHex(bw, 0xFFD8); // JPEG INIT
            DrawingUtils.WriteHex(bw, marker);
            DrawingUtils.WriteHex(bw, length);
            bw.Write('J');
            bw.Write('F');
            bw.Write('I');
            bw.Write('F');
            ...
            bw.Write(versionlo);
            bw.Write(xyunits);
            DrawingUtils.WriteHex(bw, xdensity);
            DrawingUtils.WriteHex(bw, ydensity);
      ...
      public class SOF0infotype
      {
         ...
         public void write_S0FInfo(BinaryWriter bw, int wid, int ht)
         {
            ...
            DrawingUtils.WriteHex(bw, length);
            bw.Write(precision);
            DrawingUtils.WriteHex(bw, ht);
            DrawingUtils.WriteHex(bw, wid);
      ...
      public class DQTinfotype
      {
         ...
         public void write_DQT(BinaryWriter bw)
         {
            ...
            DrawingUtils.WriteHex(bw, length);
            bw.Write(QTYinfo);
            DrawingUtils.WriteByteArray(bw, JpegTables.Y_Table, 0);
            bw.Write(QTCbinfo);
            DrawingUtils.WriteByteArray(bw, JpegTables.CbCr_Table, 0);
         }
      };
      ...
      public class DHTinfotype
      {
         ...
         byte[] YDC_nrcodes = JpegTables.Standard_DC_Luminance_NRCodes; //at index i = nr of codes with length i
         byte[] YDC_values = JpegTables.Standard_DC_Luminance_Values;
         byte HTYACinfo = 0x10; // = 0x10
         byte[] YAC_nrcodes = JpegTables.Standard_AC_Luminance_NRCodes;
         byte[] YAC_values = JpegTables.Standard_AC_Luminance_Values;//we'll use the standard Huffman tables
         byte HTCbDCinfo = 0x01; // = 1
         byte[] CbDC_nrcodes = JpegTables.Standard_DC_Chromiance_NRCodes;
         byte[] CbDC_values = JpegTables.Standard_DC_Chromiance_Values;
         byte HTCbACinfo = 0x11; //  = 0x11
         byte[] CbAC_nrcodes = JpegTables.Standard_AC_Chromiance_NRCodes;
         byte[] CbAC_values = JpegTables.Standard_AC_Chromiance_Values;
         public void write_DHT(BinaryWriter bw)
         {
            DrawingUtils.WriteHex(bw, marker);
            DrawingUtils.WriteHex(bw, length);
            bw.Write(HTYDCinfo);
            DrawingUtils.WriteByteArray(bw, YDC_nrcodes, 1);
            DrawingUtils.WriteByteArray(bw, YDC_values, 0);
            bw.Write(HTYACinfo);
            DrawingUtils.WriteByteArray(bw, YAC_nrcodes, 1);
            DrawingUtils.WriteByteArray(bw, YAC_values, 0);
            bw.Write(HTCbDCinfo);
            DrawingUtils.WriteByteArray(bw, CbDC_nrcodes, 1);
            DrawingUtils.WriteByteArray(bw, CbDC_values, 0);
            bw.Write(HTCbACinfo);
            DrawingUtils.WriteByteArray(bw, CbAC_nrcodes, 1);
            DrawingUtils.WriteByteArray(bw, CbAC_values, 0);
         }
      };
