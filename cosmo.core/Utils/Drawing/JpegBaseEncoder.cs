...
namespace Cosmo.Utils.Drawing
{
   ...
   /// </summary>
   internal class JpegBaseEncoder
   {
      sbyte[] Y_Data = new sbyte[64];
      sbyte[] Cb_Data = new sbyte[64];
      sbyte[] Cr_Data = new sbyte[64];

      private byte[] _luminance_table = JpegTables.Standard_Luminance_Quantization_Table;
      private byte[] _chromiance_table = JpegTables.Standard_Chromiance_Quantization_Table;
      ...
      public void EncodeImageBufferToJpg(Byte[, ,] ImageBuffer, Point originalDimension, Point actualDimension, BinaryWriter OutputStream, float Quantizer_Quality, DrawingUtils.IProgress progress, DrawingUtils.ICurrentOperation currentOperation)
      {
         ...
         currentOperation.SetOperation(DrawingUtils.CurrentOperation.InitializingTables);
         JpegTables.InitializeAllTables(Quantizer_Quality, _luminance_table, _chromiance_table);
         currentOperation.SetOperation(DrawingUtils.CurrentOperation.WritingJPEGHeader);
         ...
         currentOperation.SetOperation(DrawingUtils.CurrentOperation.EncodeImageBufferToJpg);
         ...
         for (ypos = 0; ypos < Height; ypos += 8)
         {
            ...
            for (xpos = 0; xpos < Width; xpos += 8)
            {
               ...
               // Process Y Channel
               Int16[] DCT_Quant_Y = Do_FDCT_Quantization_And_ZigZag(Y_Data, JpegTables.FDCT_Y_Quantization_Table);
               DoHuffmanEncoding(DCT_Quant_Y, ref prev_DC_Y, JpegTables.Y_DC_Huffman_Table, JpegTables.Y_AC_Huffman_Table, OutputStream);

               // Process Cb Channel
               Int16[] DCT_Quant_Cb = Do_FDCT_Quantization_And_ZigZag(Cb_Data, JpegTables.FDCT_CbCr_Quantization_Table);
               DoHuffmanEncoding(DCT_Quant_Cb, ref prev_DC_Cb, JpegTables.Cb_DC_Huffman_Table, JpegTables.Cb_AC_Huffman_Table, OutputStream);

               // Process Cr Channel
               Int16[] DCT_Quant_Cr = Do_FDCT_Quantization_And_ZigZag(Cr_Data, JpegTables.FDCT_CbCr_Quantization_Table);
               DoHuffmanEncoding(DCT_Quant_Cr, ref prev_DC_Cr, JpegTables.Cb_DC_Huffman_Table, JpegTables.Cb_AC_Huffman_Table, OutputStream);
            }
         }
         DrawingUtils.WriteHex(OutputStream, 0xFFD9); //Write End of Image Marker    

         currentOperation.SetOperation(DrawingUtils.CurrentOperation.Ready);
      }
      ...
      /// <param name="progress">Interface for updating Progress.</param>
      /// <param name="currentOperation">Interface for updating CurrentOperation.</param>
      public void EncodeImageToJpg(Image ImageToBeEncoded, BinaryWriter OutputStream, float Quantizer_Quality, DrawingUtils.IProgress progress, DrawingUtils.ICurrentOperation currentOperation)
      {
         Bitmap b_in = new Bitmap(ImageToBeEncoded);
         Width = b_in.Width;
         Height = b_in.Height;
         Point originalSize = new Point(b_in.Width, b_in.Height);
         currentOperation.SetOperation(DrawingUtils.CurrentOperation.FillImageBuffer);

         Bitmap_RGB_Buffer = DrawingUtils.FillImageBuffer(b_in, progress, currentOperation);

         EncodeImageBufferToJpg(Bitmap_RGB_Buffer, originalSize, DrawingUtils.GetActualDimension(originalSize), OutputStream,
      ...
      private Int16[] Do_FDCT_Quantization_And_ZigZag(SByte[] channel_data, float[] quant_table)
      {
         ...
         for (i = 0; i < 64; i++)
         {
            ...
            outdata[JpegTables.ZigZag[i]] = (Int16)((Int16)(temp + 16384.5) - 16384);
         }
      ...
      private void Update_Global_Pixel_8_8_Data(int posX, int posY)
      {
         for (int i = 0; i < 8; i++)
         {
            for (int j = 0; j < 8; j++)
            {
               ...
               Y_Data[i + j * 8] = (sbyte)(((JpegTables.Y_Red_Table[(R)] + JpegTables.Y_Green_Table[(G)] + JpegTables.Y_Blue_Table[(B)]) >> 16) - 128);
               Cb_Data[i + j * 8] = (sbyte)((JpegTables.Cb_Red_Table[(R)] + JpegTables.Cb_Green_Table[(G)] + JpegTables.Cb_Blue_Table[(B)]) >> 16);
               Cr_Data[i + j * 8] = (sbyte)((JpegTables.Cr_Red_Table[(R)] + JpegTables.Cr_Green_Table[(G)] + JpegTables.Cr_Blue_Table[(B)]) >> 16);
            }
         }
      }

      private void DoHuffmanEncoding(Int16[] DU, ref Int16 DC, JpegTables.BitString[] HTDC, JpegTables.BitString[] HTAC, BinaryWriter bw)
      {
         JpegTables.BitString EOB = HTAC[0x00];
         JpegTables.BitString M16zeroes = HTAC[0xF0];
         Byte i;
         Byte startpos;
         Byte end0pos;
         Byte nrzeroes;
         ...
         {
            WriteBits(HTDC[JpegTables.Category[32767 + Diff]], bw);
            WriteBits(JpegTables.BitCode[32767 + Diff], bw);
         }
         ...
         while (i <= end0pos)
         {
            ...
            WriteBits(HTAC[nrzeroes * 16 + JpegTables.Category[32767 + DU[i]]], bw);
            WriteBits(JpegTables.BitCode[32767 + DU[i]], bw);
            i++;
         }
      ...
      private void WriteBits(JpegTables.BitString bs, BinaryWriter bw)
      {
         UInt16 value;
         SByte posval;
