...
namespace Cosmo.Utils.Drawing
{
    ...
    internal class Imaging
    {
        ...
        public static byte[,,] GetChannelData(byte[, ,] RGB_Buffer,bool drawInGrayscale, int width, int height, ChannelType channel, DrawingUtils.IProgress progress, DrawingUtils.ICurrentOperation operation)
        {
            operation.SetOperation(DrawingUtils.CurrentOperation.GetChannelData);
            ...
            {                    
                ...
                case ChannelType.Y:
                    ...
                    for (int i = 0; i < width; i++)
                    {
                        ...
                        for (int j = 0; j < height; j++)
                        { 
                            outData[i, j, 0] = ((Byte)  (((JpegTables.Y_Red_Table[(RGB_Buffer[i, j, 0])] + JpegTables.Y_Green_Table[(RGB_Buffer[i, j, 1])] + JpegTables.Y_Blue_Table[(RGB_Buffer[i, j, 2])]) >> 16) - 128));
                ...
                case ChannelType.Cb:
                    ...
                    for (int i = 0; i < width; i++)
                    {
                        ...
                        for (int j = 0; j < height; j++)
                        {
                            outData[i, j, 0] = ((Byte)((JpegTables.Cb_Red_Table[(RGB_Buffer[i, j, 0])] + JpegTables.Cb_Green_Table[(RGB_Buffer[i, j, 1])] + JpegTables.Cb_Blue_Table[(RGB_Buffer[i, j, 2])]) >> 16));
                            outData[i, j, 1] = outData[i, j, 0];
                            outData[i, j, 2] = outData[i, j, 0];
                        }
                    }
                ...
                case ChannelType.Cr:
                    ...
                    for (int i = 0; i < width; i++)
                    {
                        ...
                        for (int j = 0; j < height; j++)
                        {
                            outData[i, j, 0] = ((Byte)((JpegTables.Cr_Red_Table[(RGB_Buffer[i, j, 0])] + JpegTables.Cr_Green_Table[(RGB_Buffer[i, j, 1])] + JpegTables.Cr_Blue_Table[(RGB_Buffer[i, j, 2])]) >> 16));
                            outData[i, j, 1] = outData[i, j, 0];
                            outData[i, j, 2] = outData[i, j, 0];
                        }
                    }
            ...
            operation.SetOperation(DrawingUtils.CurrentOperation.Ready);
            return outData;       
        }        
    }
}
